...
export class AsesorauthGuard implements CanActivate {
  ...
  canActivate(route: ActivatedRouteSnapshot, state: RouterStateSnapshot) {
    ...
    let currentToken =
      this.cookieService.get("atoken") !== ""
        ? JSON.parse(this.cookieService.get("atoken"))
        : null;
    if (currentToken) {
    ...
    this.router.navigate(["/loginAsesor"]); // , { queryParams: { returnUrl: state.url } }
    // console.log("validateToken", this.validateToken);
    return false;
  }

  private validate(currentToken: Token, tokenType: string) {
